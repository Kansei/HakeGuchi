<template>
  <div>
    ...
    <div>
      <ul class="collection">
        <li v-for="task in tasks"  :key="'row_task_' + task.id" class="collection-item">
          <input type="checkbox" v-if="!task.is_done" v-bind:id="'task_'+task.id" />
          <label  v-if="!task.is_done" v-bind:for="'task_' + task.id">{{ task.name }}</label>
        </li>
      </ul>
    </div>
    <!-- 完了済みタスク表示ボタン -->
    ...
    <div id="finished-tasks" class="display_none">
      <ul class="collection">
        <li v-for="task in tasks" :key="'row_task_'+task.id" class="collection-item">
          <input type="checkbox" v-if="task.is_done" v-bind:id="'task_'+task.id" checked="checked" />
          <label v-if="task.is_done" v-bind:for="'task_'+task.id" class="line-through">{{ task.name }}</label>
        </li>
      </ul>
    </div>
  </div>
