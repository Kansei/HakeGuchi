<template>
  <div>
    ...
    <div>
      <ul class="collection">
        <div v-for="task in tasks" :key="'row_task_' + task.id" v-bind:id="'row_task_' + task.id">
          <li class="collection-item" v-if="!task.is_done">
            <label v-bind:for="'task_' + task.id">
              <input type="checkbox" v-on:change="doneTask(task.id)"  v-bind:id="'task_' + task.id" /><span>{{ task.name }}</span>
            </label>
          </li>
        </div>
      </ul>
    ...
    <div id="finished-tasks" class="display_none">
      <ul class="collection">
        <div v-for="task in tasks" :key="'row_task_' + task.id" v-bind:id="'row_task_' + task.id">
        <li  class="collection-item" v-if="task.is_done">
          <label class="line-through" v-bind:for="'task_' + task.id">
              <input type="checkbox" v-bind:id="'task_' + task.id" checked="checked" /><span>{{ task.name }}</span>
          </label>
        </li>
        </div>
      </ul>
  ...
  export default {
    ...
    methods: {
      ...
      moveFinishedTask: function(task_id) {
        ...
        var li = document.querySelector('#finished-tasks > ul > div > li:first-child');
        document.querySelector('#finished-tasks > ul > div').insertBefore(el_clone, li);
      }
    }
  }
</script>
